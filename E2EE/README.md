...
- **Publication outcome**: the separate, terminal answer to "was this change actually saved?", which is exactly one of `NotCommitted` (proven not saved, so your prior state stays reusable, and a retry is only a deliberate choice when the error semantics permit it), `Committed` (proven saved), or `Ambiguous` (cannot be proven either way, must be reconciled, never blindly retried).
